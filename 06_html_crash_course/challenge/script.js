...
function showQuote() {
  ...
  const newQuoteAuther = document.createElement('p');
  newQuoteText.textContent = randomQuote.quote;
  newQuoteAuther.textContent = `- ${randomQuote.author}`;

  clearAll();

  quoteContainer.appendChild(newQuoteText);
  quoteContainer.appendChild(newQuoteAuther);
}
...
function revealAnswers() {
  ...
  // query in Element riddleContent rather than document
  const riddle = riddleContent.querySelector('p');
  const riddleAnser = document.querySelector('#riddle-answer');

  if (!riddle || !riddleAnser) {
    alert('There is no riddle to show the answer for!')
  } else if (!riddleAnser.hidden) {
    alert('The riddle answer is already exposed!')
  } else {
    riddleAnser.hidden = false;
  }
}
